    ...
    <el-table v-loading="loading" :data="orderList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="维修状态" align="center" prop="state">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.dor_repair_state" :value="scope.row.state"/>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getOrder(repairId).then(response => {
    ...
    handleDelete(row) {
      const repairId = row.repairId || this.ids;
      this.$modal.confirm('是否确认删除维修工单编号为"' + repairId + '"的数据项？').then(function () {
        return delOrder(repairId);
      }).then(() => {
        this.getList();
