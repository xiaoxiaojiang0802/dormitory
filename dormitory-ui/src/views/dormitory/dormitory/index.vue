...
<script>
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getDormitory(createBy).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除宿舍编号为"' + createBys + '"的数据项？').then(function () {
        return delDormitory(createBys);}).then(() => {
        this.getList();
  ...
  /** 导出按钮操作 */
  handleExport(){
    this.download('pm/dormitory/export', {
      ...this.queryParams
