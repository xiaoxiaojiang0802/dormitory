...
import com.cdmzl.common.actable.constants.*;
import com.cdmzl.common.actable.mapper.CreateH2TablesMapper;
 ...
 * @version 2019/07/06
 */
@Service("sysH2CreateTableService")
public class SysH2CreateTableServiceImpl implements SysH2CreateTableService {

    private static final Logger log = LoggerFactory.getLogger(SysH2CreateTableServiceImpl.class);

    @Autowired
    private CreateH2TablesMapper createH2TablesMapper;
    ...
    public void createH2Table() {
        /*要扫描的model所在的pack*/
        String pack = springContextUtil.getConfig(AcTableConstants.MODEL_PACK_KEY);
        tableAuto = springContextUtil.getConfig(AcTableConstants.TABLE_AUTO_KEY);
        // 不做任何事情
    ...
    private Map<String, Map<String, TableConfig>> initTableMap() {
        ...
        baseTableMap.put(AcTableConstants.NEW_TABLE_MAP, new HashMap<String, TableConfig>());
        // 2.用于存需要更新字段类型等的表名+结构
        baseTableMap.put(AcTableConstants.MODIFY_TABLE_MAP, new HashMap<String, TableConfig>());
        // 3.用于存需要增加字段的表名+结构
        baseTableMap.put(AcTableConstants.ADD_TABLE_MAP, new HashMap<String, TableConfig>());
        // 4.用于存需要删除字段的表名+结构
        baseTableMap.put(AcTableConstants.REMOVE_TABLE_MAP, new HashMap<String, TableConfig>());
        // 5.用于存需要删除主键的表名+结构
        baseTableMap.put(AcTableConstants.DROPKEY_TABLE_MAP, new HashMap<String, TableConfig>());
        // 6.用于存需要删除唯一约束的表名+结构
        baseTableMap.put(AcTableConstants.DROPINDEXANDUNIQUE_TABLE_MAP, new HashMap<String, TableConfig>());
        // 7.用于存需要增加的索引
        baseTableMap.put(AcTableConstants.ADDINDEX_TABLE_MAP, new HashMap<String, TableConfig>());
        // 8.用于存需要增加的唯一约束
        baseTableMap.put(AcTableConstants.ADDUNIQUE_TABLE_MAP, new HashMap<String, TableConfig>());
        // 9.更新表注释
        baseTableMap.put(AcTableConstants.MODIFY_TABLE_PROPERTY_MAP, new HashMap<String, TableConfig>());
        return baseTableMap;
    }
    ...
    private void buildTableMapConstruct(Class<?> clas, Map<String, Map<String, TableConfig>> baseTableMap) {
        ...
        if ("create".equals(tableAuto)) {
            ...
            createH2TablesMapper.dropTableByName(tableName);
        }

        // 先查该表是否以存在
        SysMysqlTable table = createH2TablesMapper.findTableByTableName(tableName);
        ...
        if (table == null) {
            ...
            baseTableMap.get(AcTableConstants.NEW_TABLE_MAP).put(tableName, new TableConfig(allFieldList, map));
            baseTableMap.get(AcTableConstants.ADDINDEX_TABLE_MAP).put(tableName, new TableConfig(getAddIndexList(null, allFieldList)));
            baseTableMap.get(AcTableConstants.ADDUNIQUE_TABLE_MAP).put(tableName, new TableConfig(getAddUniqueList(null, allFieldList)));
            return;
        } else {
            Map<String, Object> map = new HashMap<String, Object>();
            // 判断表注释是否要更新
            ...
            baseTableMap.get(AcTableConstants.MODIFY_TABLE_PROPERTY_MAP).put(tableName, new TableConfig(map));
        }

        // 已存在时理论上做修改的操作，这里查出该表的结构
        List<SysMysqlColumns> tableColumnList = createH2TablesMapper.findTableEnsembleByTableName(tableName);
        ...
        String uniPrefix = springContextUtil.getConfig(AcTableConstants.TABLE_UNIQUE_KEY);
        String idxPrefix = springContextUtil.getConfig(AcTableConstants.TABLE_INDEX_KEY);
        ...
        Set<String> allIndexAndUniqueNames = createH2TablesMapper.findTableIndexByTableName(paramMap);
        ...
        if (addFieldList.size() != 0) {
            baseTableMap.get(AcTableConstants.ADD_TABLE_MAP).put(tableName, new TableConfig(addFieldList));
        }
        if (removeFieldList.size() != 0) {
            baseTableMap.get(AcTableConstants.REMOVE_TABLE_MAP).put(tableName, new TableConfig(removeFieldList));
        }
        if (modifyFieldList.size() != 0) {
            baseTableMap.get(AcTableConstants.MODIFY_TABLE_MAP).put(tableName, new TableConfig(modifyFieldList));
        }
        if (dropKeyFieldList.size() != 0) {
            baseTableMap.get(AcTableConstants.DROPKEY_TABLE_MAP).put(tableName, new TableConfig(dropKeyFieldList));
        }
        if (dropIndexAndUniqueFieldList.size() != 0) {
            baseTableMap.get(AcTableConstants.DROPINDEXANDUNIQUE_TABLE_MAP).put(tableName, new TableConfig(dropIndexAndUniqueFieldList));
        }
        if (addIndexFieldList.size() != 0) {
            baseTableMap.get(AcTableConstants.ADDINDEX_TABLE_MAP).put(tableName, new TableConfig(addIndexFieldList));
        }
        if (addUniqueFieldList.size() != 0) {
            baseTableMap.get(AcTableConstants.ADDUNIQUE_TABLE_MAP).put(tableName, new TableConfig(addUniqueFieldList));
        }
    }
...
//                if (!"PRI".equals(sysColumn.getColumn_key()) && createTableParam.isFieldIsKey()) {
//                    log.warn(sysColumn.getColumn_key() + "------1------" + createTableParam.isFieldIsKey());
...
//                }
                ...
                // 2.验证类型
                if (!sysColumn.getData_type().toLowerCase().equals(H2DataTypeMap.getDataType(createTableParam.getFieldType().toLowerCase()))) {
                    modifyFieldList.add(modifyTableParam);
                    log.warn(sysColumn.getData_type().toLowerCase() + "------3------" + createTableParam.getFieldType().toLowerCase());
                    continue;
                }
                // 3.验证长度个小数点位数
                ...
                if (createTableParam.getFileTypeLength() == 1) {
                    ...
                    if (!"bigint".equals(typeAndLength) && !"int".equals(typeAndLength)) {
                ...
                // 判断类型+长度是否相同
                if (!sysColumn.getColumn_type().toLowerCase().startsWith(typeAndLength)) {
                    modifyFieldList.add(modifyTableParam);
                    log.warn(sysColumn.getColumn_type().toLowerCase() + "------4------" + typeAndLength);
                    continue;
                }
                // 5.验证自增
                ...
                if (sysColumn.getColumn_default() == null || sysColumn.getColumn_default().equals("")) {
                    ...
                    if (H2DataTypeMap.VARCHAR.toString().toLowerCase().equals(createTableParam.getFieldType()) && !createTableParam.isFieldDefaultValueNative()) {
                        ...
                        if (("false".equals(createTableParam.getFieldDefaultValue()) || "?1".equals(createTableParam.getFieldDefaultValue()))
                                && !"b'0'".equals(sysColumn.getColumn_default())) {
                            // 两者不相等时，需要更新该字段
                            modifyFieldList.add(modifyTableParam);
                            log.warn(createTableParam.getFieldDefaultValue() + "------8------" + sysColumn.getColumn_default());
                            continue;
                        }
                    } else {
                        // 两者不相等时，需要更新该字段
                        modifyFieldList.add(modifyTableParam);
                        log.warn(MySqlTypeConstant.BIT.toString().toLowerCase() + "------9--1----" + createTableParam.getFieldType());
                        log.warn(MySqlTypeConstant.BIT.toString().toLowerCase() + "------9--2----" + createTableParam.isFieldDefaultValueNative());
                        continue;
                    }
                }
                ...
                // 8.验证注释
//                if (sysColumn.getColumn_comment() != null && !sysColumn.getColumn_comment().equals(createTableParam.getFieldComment())) {
//                    modifyFieldList.add(modifyTableParam);
//                }
            }
        }
    ...
    private void createOrModifyTableConstruct(Map<String, Map<String, TableConfig>> baseTableMap) {
        ...
        // 1. 创建表
        createTableByMap(baseTableMap.get(AcTableConstants.NEW_TABLE_MAP));

        // add是追加模式不做删除和修改操作
        if (!"add".equals(tableAuto)) {
            // 2. 删除要变更主键的表的原来的字段的主键
            dropFieldsKeyByMap(baseTableMap.get(AcTableConstants.DROPKEY_TABLE_MAP));
        }

        // add是追加模式不做删除和修改操作
        if (!"add".equals(tableAuto)) {
            // 3. 删除索引和约束
            dropIndexAndUniqueByMap(baseTableMap.get(AcTableConstants.DROPINDEXANDUNIQUE_TABLE_MAP));
            // 4. 删除字段
            removeFieldsByMap(baseTableMap.get(AcTableConstants.REMOVE_TABLE_MAP));
            // 5. 修改表注释
            modifyTableCommentByMap(baseTableMap.get(AcTableConstants.MODIFY_TABLE_PROPERTY_MAP));
            // 6. 修改字段类型等
            modifyFieldsByMap(baseTableMap.get(AcTableConstants.MODIFY_TABLE_MAP));
        }

        // 7. 添加新的字段
        addFieldsByMap(baseTableMap.get(AcTableConstants.ADD_TABLE_MAP));

        // 8. 创建索引
        addIndexByMap(baseTableMap.get(AcTableConstants.ADDINDEX_TABLE_MAP));

        // 9. 创建约束
        addUniqueByMap(baseTableMap.get(AcTableConstants.ADDUNIQUE_TABLE_MAP));
        ...
        if (dropIndexAndUniqueMap.size() > 0) {
            for (Entry<String, TableConfig> entry : dropIndexAndUniqueMap.entrySet()) {
                for (Object obj : entry.getValue().getList()) {
                    ...
                    createH2TablesMapper.dropTabelIndex(map);
            ...
            for (Entry<String, TableConfig> entry : addIndexMap.entrySet()) {
                for (Object obj : entry.getValue().getList()) {
                    ...
                    if (null != fieldProperties.getFiledIndexName()) {
                        ...
                        createH2TablesMapper.addTableIndex(map);
            ...
            for (Entry<String, TableConfig> entry : addUniqueMap.entrySet()) {
                for (Object obj : entry.getValue().getList()) {
                    ...
                    if (null != fieldProperties.getFiledUniqueName()) {
                        ...
                        createH2TablesMapper.addTableUnique(map);
        ...
        if (modifyTableMap.size() > 0) {
            for (Entry<String, TableConfig> entry : modifyTableMap.entrySet()) {
                for (Object obj : entry.getValue().getList()) {
                    ...
                    createH2TablesMapper.modifyTableField(map);
        ...
        if (removeTableMap.size() > 0) {
            for (Entry<String, TableConfig> entry : removeTableMap.entrySet()) {
                for (Object obj : entry.getValue().getList()) {
                    ...
                    createH2TablesMapper.removeTableField(map);
        ...
        if (modifyTableCommentMap.size() > 0) {
            for (Entry<String, TableConfig> entry : modifyTableCommentMap.entrySet()) {
                for (String property : entry.getValue().getMap().keySet()) {
                    ...
                    createH2TablesMapper.modifyTableProperty(map);
        ...
        if (addTableMap.size() > 0) {
            for (Entry<String, TableConfig> entry : addTableMap.entrySet()) {
                for (Object obj : entry.getValue().getList()) {
                    ...
                    createH2TablesMapper.addTableField(map);
        ...
        if (dropKeyTableMap.size() > 0) {
            for (Entry<String, TableConfig> entry : dropKeyTableMap.entrySet()) {
                for (Object obj : entry.getValue().getList()) {
                    ...
                    createH2TablesMapper.dropKeyTableField(map);
    ...
    private void createTableByMap(Map<String, TableConfig> newTableMap) {
        ...
        if (newTableMap.size() > 0) {
            for (Entry<String, TableConfig> entry : newTableMap.entrySet()) {
                ...
                createH2TablesMapper.createTable(map);
