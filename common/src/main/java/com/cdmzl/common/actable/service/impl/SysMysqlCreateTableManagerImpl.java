...
public class SysMysqlCreateTableManagerImpl implements SysMysqlCreateTableManager {
    ...
    public void createMysqlTable() {
        // 读取配置信息
        pack = springContextUtil.getConfig(AcTableConstants.MODEL_PACK_KEY);
    ...
    public List<Object> getAllFields(Class<?> clas) {
        ...
        for (Field field : fields) {
            ...
            // 判断方法中是否有指定注解类型的注解
            if (ColumnUtils.hasColumnAnnotation(field, clas) && ColumnUtils.notExist(field)) {
                ...
                if (null != index) {
                    ...
                    param.setFiledIndexName((index.value() == null || index.value().equals("")) ? (idxPrefix + ((indexValue.length == 0) ? ColumnUtils.getColumnName(field, clas) : stringArrFormat(indexValue))) : idxPrefix + index.value());
                ...
                if (null != unique) {
                    ...
                    param.setFiledUniqueName((unique.value() == null || unique.value().equals("")) ? (uniPrefix + ((uniqueValue.length == 0) ? ColumnUtils.getColumnName(field, clas) : stringArrFormat(uniqueValue))) : uniPrefix + unique.value());
